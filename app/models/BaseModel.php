...
abstract class BaseModel
{
    ...
    {
        $pdo = static::db();
        $query = $pdo->prepare('SELECT * FROM ' . static::$table . ' WHERE id = :id LIMIT 1');
        $query->execute(['id' => $id]);
    ...
    {
        $pdo = static::db();
        $query = $pdo->prepare('DELETE FROM ' . static::$table . ' WHERE id = :id');
    ...
    {
        $pdo = static::db();
        $query = $pdo->query('SELECT * FROM ' . static::$table . ' ORDER BY id DESC LIMIT ' . (int)$limit);
    ...
    public static function count(): int
    {
        ...
        return (int)$pdo->query('SELECT COUNT(*) FROM ' . static::$table)->fetchColumn();
    }
    ...
    {
        $pdo = static::db();
        $query = $pdo->prepare('DELETE FROM ' . static::$table . " WHERE $column = :value");
